...
Battery::Battery()
{
	...
	startDecreaseBattery(); // Bắt đầu luồng để giảm pin
	...
	auto decreaseFunction = [this]()
	{
		while (true)
		{
			...
			if (currentBatteryCapacity > minBatteryCapacity)
			{
				...
				setBatteryLevel(minBatteryCapacity, maxBatteryCapacity, currentBatteryCapacity); // Cập nhật batteryLevel
			}
...
void MainMenu::rightTurn()
{
	for (int i = 0; i < 3; i++)
	{
		cout << setw(24) << setfill(' '); // Đặt độ rộng cố định và điền bằng khoảng trắng
		for (int k = 0; k < i; k++)
		{
			cout << " ";
		}
